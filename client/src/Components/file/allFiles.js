...
function AllFiles() {
  ...
  const ResultItemTemplate = (options) => {
    return options == 1 ? (
      <i
        style={{ color: "green" }}
        className={classNames("pi", "true-icon pi-check-circle")}
      ></i>
    ) : options == 0 ? (
      <i
        style={{ color: "red" }}
        className={classNames("pi", "false-icon pi-times-circle")}
      ></i>
    ) : (
  ...
  return (
    <div>
      <Toolbar
        ...
        left={LeftToolbarTemplate(selectedFiles, refetch, navigate)}
      ></Toolbar>
      <DataTable
        ref={dt}
        value={files}
        selection={selectedFiles}
        onRowDoubleClick={EnterFile}
        onSelectionChange={(e) => {
      ...
      >
        ...
        <Column
          className="text-right"
          field="IDnumberOfApplicant"
          filter
          filterPlaceholder="חיפוש"
          style={{ minWidth: "12rem" }}
          header='ת"ז מגיש הבקשה'
          sortable
        ></Column>
        <Column
          className="text-right"
